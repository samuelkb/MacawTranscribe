...
@csrf_exempt
@require_POST
def upload_normalize_diarize_vad_and_chunk_recording_view(request: HttpRequest) -> JsonResponse:
    """
    Upload a recording and run the first five pipeline stages:
    ...
    try:
        result = upload_normalize_diarize_vad_and_chunk_recording(uploaded_file=uploaded_file)
    except ValueError as exc:
        logger.warning(
            "pipeline_upload_rejected",
            extra={
